 ...
 *
 *      ══════════════════════════════════════════════════════════════════════
 *      ⚠ WHAT CHANGED IN THIS CONTRACT: BNB IS NATIVE, END TO END
 *      ══════════════════════════════════════════════════════════════════════
 *      The previous Duel settled BNB fights in WBNB, for two reasons that were
 *      both real:
 *
 *        1. A stake can only be pulled from a PASSIVE opponent by allowance,
 *           and raw BNB cannot grant one. "Physics, not policy."
 *        2. A raw-BNB payout is a `.call{value:}` to whoever won, and a winner
 *           that is a contract with a reverting `receive()` would revert the
 *           whole duel — a griefing vector that settles nothing and kills the
 *           fight. `safeTransfer` cannot be refused.
 *
 *      Both are answered by ONE mechanism: a custodied native balance,
 *      `bnbCredit`. Allowance is unnecessary when the contract already holds
 *      the money, and a payout that is a storage write cannot be refused
 *      either — the winner pulls it later with `withdraw`, where a broken
 *      `receive()` only ever hurts its own owner.
 *
 *      So the player deposits BNB, fights in BNB, wins BNB and withdraws BNB.
 *      **No player ever holds, approves or receives a wrapped token.**
 *
 *      ⚠ ONE WRAP SURVIVES, and it is not player-facing: `Jackpot.prizeToken`
 *      is `immutable` WBNB, so the dev-cut slice routed into the BNB pot is
 *      wrapped at that boundary and nowhere else — `routePotSliceInline`. When
 *      the jackpots are redeployed to settle natively, that wrap disappears
 *      and WBNB leaves the system entirely. Until then a JACKPOT winner is
 *      still paid in WBNB by `Jackpot.sol`, which this contract cannot fix.
 *
 *      ⚠ THE CONTRACT NOW CUSTODIES PLAYER MONEY BETWEEN TRANSACTIONS, which
 *      the previous one never did. Every invariant that leaned on "this
 *      contract holds nothing at rest" has been revisited — see `rescueToken`
 *      and the solvency note on `totalBnbCredit`.
 *
 *      ══════════════════════════════════════════════════════════════════════
 ...
 */
contract DuelNative is Ownable, Pausable, ReentrancyGuard, EIP712 {
    ...
    // ─── The native BNB credit ledger ────────────────────────────────────

    /// @notice A wallet's custodied BNB, in wei. Deposited by the player,
    ///         spent by a passive stake, earned by winning, withdrawn at will.
    ///
    /// @dev ⚠ THIS IS WHY THE CONTRACT EXISTS. Native BNB has no allowance
    ///      primitive, so the ONLY way to charge a wallet that is not signing
    ///      the transaction is to already be holding its money. The previous
    ///      Duel solved that with a WBNB allowance, which meant players held
    ///      and were paid in a wrapped token they never asked for. This ledger
    ///      replaces it: BNB in, BNB out, nothing wrapped in between.
    ///
    ///      ⚠ IT ALSO KILLS A GRIEFING VECTOR the previous contract could only
    ///      dodge. There, the winner was PUSHED their prize; a winner that is a
    ///      contract with a reverting `receive()` would have reverted the whole
    ///      duel, so the pot had to settle in WBNB because `safeTransfer`
    ///      cannot be refused. Here the payout is a storage write, which cannot
    ///      revert at all, and the winner pulls with `withdraw` in their own
    ///      transaction. A broken `receive()` now only ever hurts its owner.
    mapping(address => uint256) public bnbCredit;

    /// @notice Sum of every entry in `bnbCredit`. Maintained on every mutation
    ///         so solvency is one comparison rather than an iteration.
    /// @dev THE SOLVENCY INVARIANT, asserted in the test suite:
    ///          address(this).balance >= totalBnbCredit
    ///      at rest between transactions. It can exceed it — a stray donation,
    ///      or a dev cut mid-flight — but must never fall below, or somebody's
    ///      withdrawal is somebody else's money.
    uint256 public totalBnbCredit;

    /// @notice The most a wallet's fight balance may be spent by a fight IT IS
    ///         NOT SIGNING. Zero means "nobody may challenge my bulls while I
    ///         am away"; the balance is still fully usable for fights the owner
    ///         submits themselves, and always fully withdrawable.
    ///
    /// @dev ⚠ THIS EXISTS BECAUSE CUSTODY DELETED THE OLD BOUND, AND NOBODY
    ///      NOTICED. On the WBNB contract a passive side paid by `transferFrom`,
    ///      so the wallet's exposure was the ERC-20 allowance it had granted —
    ///      a number the player chose, that the site set to fifty fights' worth.
    ///      Moving to a credit ledger removed the allowance and silently
    ///      replaced that ceiling with THE ENTIRE FLOAT.
    ///
    ///      That matters because `submitDuel` settles a result signed off
    ///      chain: whoever holds the signer key names both bulls, both stakes
    ///      and the winner. An external review demonstrated the consequence —
    ///      a leaked signer moved 81 of 90 BNB out of a depositor in ONE
    ///      transaction and withdrew it, bounded by nothing but
    ///      `maxFightCostOf`, which bounds a single fight and not the total.
    ///      `pause()` covers a leaked SIGNER; it does not cover a leaked owner
    ///      key, which can call `setTrustedSigner` and is one transaction away.
    ///
    ///      Restoring an explicit, player-set ceiling restores the property the
    ///      old design had for free. It costs the player nothing per fight —
    ///      they set it once, exactly as they once approved once — so the
    ///      offline-challenge product is unchanged.
    mapping(address => uint256) public passiveAllowance;
    ...
    /// ⚠ `NativeStakeWrapped` is DELETED, not renamed. In the previous Duel it
    ///   fired whenever a stake was wrapped into WBNB; here stakes are never
    ///   wrapped, so an event by that name could only ever lie. The one
    ///   surviving wrap has its own event, `PotSliceWrapped`.

    /// @notice BNB was added to a wallet's custodied fight balance.
    event CreditDeposited(address indexed wallet, uint256 amount, uint256 balance);
    /// @notice A wallet pulled BNB back out of its fight balance.
    event CreditWithdrawn(address indexed wallet, uint256 amount, uint256 balance);
    /// @notice A stake was taken from a wallet's custodied balance rather than
    ///         from `msg.value` — i.e. the passive side of a fight.
    event CreditSpent(address indexed wallet, uint256 amount, uint256 balance);

    /// @notice A wallet DELIBERATELY changed its ceiling. Owner action only.
    event PassiveAllowanceSet(address indexed wallet, uint256 allowance);

    /// @notice A fight consumed part of a wallet's ceiling. Emitted only by the
    ///         automatic decrement, never by the owner's own setter, so a
    ///         monitor can tell a budget being eaten from one being lowered.
    event PassiveAllowanceConsumed(
        address indexed wallet, uint256 spent, uint256 remaining
    );
    /// @notice Winnings, a tie refund or a dev cut landed as credit. The payee
    ///         withdraws in their own transaction; see `_creditBnb`.
    event CreditEarned(address indexed wallet, uint256 amount, uint256 balance);
    /// @notice THE LAST WRAP IN THE CONTRACT. `Jackpot.prizeToken` is an
    ///         immutable WBNB, so the pot slice — and only the pot slice — is
    ///         wrapped at the boundary before being funded in.
    event PotSliceWrapped(uint256 amount);
    ...
    /// @dev The consent gate is unwired or points at a non-contract. Fights are
    ///      refused until it is wired — see `_requireInYards` for why this is
    ///      fail-closed in a contract that custodies player money.
    error YardsNotWired(address slot);
    /// @dev A passive side's stake exceeded the allowance its owner set. The
    ///      wallet is not signing this transaction, so this is the only thing
    ///      standing between it and whoever holds the signer key.
    error PassiveAllowanceExceeded(address wallet, uint256 needed, uint256 allowed);
    ...
    error RefundFailed();
    /// @dev A passive side's custodied BNB credit could not cover its stake.
    ///      Distinct from `StakeUnaffordable` so the UI can say "top up your
    ///      fight balance" rather than "your wallet is short".
    error InsufficientCredit(address wallet, uint256 needed, uint256 held);
    ...
    // ══════════════════════════════════════════════════════════════════════
    //  The fight balance — deposit, withdraw
    // ══════════════════════════════════════════════════════════════════════

    /**
     * @notice Top up your fight balance so your bulls can be challenged while
     *         you are offline.
     * @dev Deliberately unguarded and unpausable. Money going IN is never the
     *      dangerous direction, and a paused deposit would strand a player
     *      mid-top-up. `withdraw` is the one that needs care.
     */
    function deposit() external payable {
        if (msg.value == 0) revert ZeroAmount();
        uint256 next = _addCredit(msg.sender, msg.value);
        emit CreditDeposited(msg.sender, msg.value, next);
    }

    /**
     * @notice Credit a wallet other than your own. For a router or a friend
     *         funding a player; the payer has no claim on it afterwards.
     */
    function depositFor(address wallet) external payable {
        if (wallet == address(0)) revert ZeroAddress();
        if (msg.value == 0) revert ZeroAmount();
        uint256 next = _addCredit(wallet, msg.value);
        emit CreditDeposited(wallet, msg.value, next);
    }

    /**
     * @notice Take your BNB back out. Any amount, any time, no lock.
     *
     * @dev ⚠ CHECKS-EFFECTS-INTERACTIONS, and the ONLY place this contract
     *      sends native to an arbitrary address on purpose. The balance is
     *      debited BEFORE the call, so a re-entrant `receive()` finds the
     *      ledger already settled and its second withdrawal reverts on its own
     *      arithmetic even if the guard were somehow absent. `nonReentrant` is
     *      the belt; CEI is the braces.
     *
     *      ⚠ NOT `whenNotPaused`. A pause is for stopping fights, not for
     *      trapping player money. A contract that can freeze withdrawals is a
     *      contract players are right not to trust.
     */
    function withdraw(uint256 amount) external nonReentrant {
        if (amount == 0) revert ZeroAmount();
        uint256 held = bnbCredit[msg.sender];
        if (held < amount) revert InsufficientCredit(msg.sender, amount, held);

        unchecked {
            bnbCredit[msg.sender] = held - amount;
            totalBnbCredit -= amount;
        }
        emit CreditWithdrawn(msg.sender, amount, held - amount);
    ...
    /**
     * @notice Set how much of your balance a fight you are NOT signing may
     *         spend. Zero switches offline challenges off entirely.
     *
     * @dev Deliberately absolute, not incremental: you are stating a ceiling,
     *      not topping one up, so there is no approve-race to reason about and
     *      re-reading it always tells you your true exposure.
     *
     *      Unpausable and unguarded for the same reason `withdraw` is — a
     *      player must always be able to REDUCE their exposure, including to
     *      zero, no matter what state the contract is in.
     */
    function setPassiveAllowance(uint256 allowance) external {
        passiveAllowance[msg.sender] = allowance;
        emit PassiveAllowanceSet(msg.sender, allowance);
    }

    /// @notice Withdraw the lot.
    function withdrawAll() external nonReentrant {
        uint256 held = bnbCredit[msg.sender];
        if (held == 0) revert ZeroAmount();

        unchecked {
            bnbCredit[msg.sender] = 0;
            totalBnbCredit -= held;
        }
        emit CreditWithdrawn(msg.sender, held, 0);

        (bool ok,) = msg.sender.call{value: held}("");
        if (!ok) revert WithdrawFailed();
    }

    /// @dev A plain BNB send is treated as a deposit rather than a donation, so
    ///      a player who transfers instead of calling `deposit()` still gets
    ///      their money credited instead of gifting it to the contract.
    ...
    /// @dev THE ONE PLACE CREDIT IS EVER CREATED. Every deposit, winning, tie
    ///      refund and dev cut funnels through here, so `totalBnbCredit` cannot
    ///      drift from the sum of the mapping no matter which path added it.
    function _addCredit(address wallet, uint256 amount) private returns (uint256 next) {
        next = bnbCredit[wallet] + amount;
        bnbCredit[wallet] = next;
        totalBnbCredit += amount;
    }

    /// @dev Winnings, tie refunds and the dev cut. Cannot revert — that is the
    ///      whole point, see the `bnbCredit` header.
    function _creditBnb(address wallet, uint256 amount) private {
        if (amount == 0) return;
        uint256 next = _addCredit(wallet, amount);
        emit CreditEarned(wallet, amount, next);
    }

    /// @dev The one place credit is ever destroyed by a stake. Reverts by name
    ///      when short so the UI can tell a player to top up.
    function _debitBnb(address wallet, uint256 amount) private {
        if (amount == 0) return;
        uint256 held = bnbCredit[wallet];
        if (held < amount) revert InsufficientCredit(wallet, amount, held);
        unchecked {
            bnbCredit[wallet] = held - amount;
            totalBnbCredit -= amount;
        }
        emit CreditSpent(wallet, amount, held - amount);
    }
     ...
     *
     *      ⚠⚠ THIS FAILS CLOSED, AND THAT IS A DELIBERATE REVERSAL OF THE
     *      INHERITED BEHAVIOUR. The WBNB-era contract skipped the check on an
     *      empty or non-contract slot, reasoning that a mis-wire should leave a
     *      degraded check rather than a dead game. That argument was written
     *      for a contract THAT CUSTODIED NOTHING, where a passive wallet's
     *      whole exposure was the ERC-20 allowance it had chosen to grant.
     *
     *      Custody breaks it. `bnbCredit` needs no allowance, and ERC-721
     *      `transferFrom` can push a bull at any address without consent — so
     *      an unwired slot lets an attacker mint junk, shove it at a depositor
     *      and fight it, spending a float that wallet never offered to anyone.
     *      Measured on the shipped migration script (which bootstraps four of
     *      the five wires and omits this one): 0.8 BNB lifted from a wallet
     *      whose only ever action was `deposit()`. `depositFor` makes it worse,
     *      letting the attacker fund the victim's side and farm any bull in the
     *      collection to death for the price of the dev cut.
     *
     *      A safety gate whose default is "off" is not a gate. The EOA-mis-wire
     *      case the old comment worried about is still handled — it reverts by
     *      NAME here rather than as an opaque `extcodesize` failure — and the
     *      "dead game" it feared is the correct state for a deployment whose
     *      consent gate is not wired yet: unfightable until somebody wires it.
     *      The frontend reads this same slot and skips its own pre-check when
     *      zero, so leaving it open disabled both gates at once.
     */
        ...
        view
    {
        ...
        if (y == address(0) || y.code.length == 0) revert YardsNotWired(y);
        ...
        returns (uint256 refund)
    {
        ...
        // ⚠ NO WRAP. This is the change. The BNB that paid a stake simply stays
        // native in this contract, backing the credit that `_distributePot` is
        // about to assign to the winner. It is wrapped exactly once, later, and
        // only if a slice of the dev cut is routed into the WBNB pot — see
        // `routePotSliceInline`, the last WBNB boundary in the system.
        refund = credit;
    }
        ...
        returns (uint256)
    {
        ...
        // ══════════════════════════════════════════════════════════════════
        //  THE BNB LEG. Native, start to finish. No allowance, no wrapping.
        // ══════════════════════════════════════════════════════════════════
        //
        // PRECEDENCE, and it matters: `msg.value` is spent FIRST, then the
        // custodied balance. A player who sends BNB with the fight pays with
        // it; one who sent none — or not enough — falls through to the balance
        // they topped up earlier. That ordering means an active player never
        // has to pre-fund, and never has money silently pulled from their
        // balance while their `msg.value` sits unspent and gets refunded.
        //
        // The passive side has no `msg.value` of its own by construction, so it
        // always lands on the credit branch. That is the entire reason this
        // ledger exists: only `msg.sender` can post value, and native BNB has
        // no allowance, so custody is the only way to charge a wallet that is
        // not signing. The old contract used a WBNB allowance here.
        if (asset == address(wbnb)) {
            // ⚠⚠ THE EXEMPTION IS THE ATTACHED VALUE, NOT THE SUBMITTER.
            //
            // A wallet paying with `msg.value` it attached itself needs no
            // ceiling: it chose the amount in the same transaction it signed.
            // Everything else — INCLUDING the submitter's own balance when the
            // value falls short — goes through the allowance.
            //
            // A revision in between exempted the submitter wholesale, on the
            // reasoning that "a fight you sign is not 'while you are away'".
            // That reasoning is appealing and it is wrong, because it removed a
            // bound and left only `maxFightCostOf[wbnb]` — one-shot, permanent,
            // 1e18 — against a live quote near 0.0033 BNB. 299x per click, and
            // silent: a compromised signer names the ceiling, the player
            // attaches the honest quote, `credit >= stake` fails, and the whole
            // stake comes off the LEDGER while the honest payment is refunded
            // untouched. The wallet balance never moves, so nothing surfaces
            // it. Measured in `test_AUDIT_submitterExemptionCosts300xTheQuotePerClick`:
            // five clicks, five BNB, away budget never touched.
            //
            // Bounding that path by the live `fighterCost` instead was the
            // other candidate and is worse than it looks: the quote is derived
            // from the oracle, so a stake signed seconds earlier reverts
            // whenever BNB ticks up in between, turning a safety bound into
            // intermittent fight failures. The attached-value rule needs no
            // oracle and cannot drift.
            //
            // `api/run-duel` already models exactly this, so the two layers
            // agree again.
            if (owner_ == msg.sender && credit >= stake) {
                unchecked {
                    return credit - stake;
                }
            }
            // ⚠ L1, DELIBERATELY NOT FIXED HERE. An external review noted that
            // `msg.value` and the balance are not COMBINED: send 0.5 against a
            // 1 BNB stake while holding 0.5 and this reverts `InsufficientCredit`
            // rather than topping up, refunding the 0.5. It is a real papercut.
            //
            // It is left alone because combining them changes what the refund
            // is, and the invariant handler models the refund arithmetically
            // (`_refundOf`). Making the change turned `invariant_creditIsNeverConjured`
            // red — correctly, because the handler's model no longer matched —
            // and I could not prove within this pass whether the residue was
            // only the stale model or a genuine accounting gap. Shipping an
            // unproven behavioural change to the money path alongside two
            // critical security fixes would make both harder to trust.
            //
            // Fix it on its own, with the handler's refund model rewritten to
            // MEASURE the refund rather than predict it.

            // ⚠ NOT THE SUBMITTER'S ATTACHED VALUE. Either this wallet is not
            // signing the transaction at all, or it is and its value fell
            // short — in both cases the money is coming off a balance that was
            // sitting there, so the ceiling it set applies. Checked and
            // DECREMENTED, so the bound is on the total taken over time and not
            // merely per fight: `maxFightCostOf` already bounds one fight, and
            // one fight is not what a leaked key does.
            uint256 allowed = passiveAllowance[owner_];
            if (allowed < stake) revert PassiveAllowanceExceeded(owner_, stake, allowed);
            unchecked {
                passiveAllowance[owner_] = allowed - stake;
            }
            // ⚠ ITS OWN EVENT, NOT `PassiveAllowanceSet`. Re-emitting the
            // user-facing one here made an automatic decrement indistinguishable
            // from the owner deliberately lowering their ceiling — and that is
            // precisely the event a wallet-safety monitor watches to notice a
            // budget draining without its owner touching it.
            emit PassiveAllowanceConsumed(owner_, stake, allowed - stake);
            _debitBnb(owner_, stake);
            return credit;
        }
    ...
    function routePotSliceInline(address asset, uint256 amount) external {
        ...
        if (asset == address(bnbull)) {
            ...
            // ══════════════════════════════════════════════════════════════
            //  ⚠⚠ THE LAST WRAP IN THE SYSTEM. THE ONLY ONE. Everything above
            //  this line is native BNB and everything a player ever touches is
            //  native BNB. It exists solely because `Jackpot.prizeToken` is
            //  `immutable` and set to WBNB in the constructor, so the pot can
            //  only be funded in WBNB until the jackpots are themselves
            //  redeployed. When they are, delete these four lines and fund the
            //  pot natively.
            //
            //  It sits INSIDE the try/catch'd leg on purpose: a wrap failure is
            //  caught upstream like any other pot fault and the slice falls
            //  back to dev, so a broken WBNB can never block a fight.
            // ══════════════════════════════════════════════════════════════
            uint256 before = IERC20(address(wbnb)).balanceOf(address(this));
            wbnb.deposit{value: amount}();
            uint256 got = IERC20(address(wbnb)).balanceOf(address(this)) - before;
            if (got < amount) revert WrapShortfall(amount, got);
            emit PotSliceWrapped(amount);
    ...
    /**
     * @dev Pay one side. Tolerates the zero-asset / zero-amount cases.
     *
     *      ⚠ THE BNB LEG IS A STORAGE WRITE, NOT A TRANSFER, and that is a
     *      safety property rather than an optimisation. The old contract had to
     *      settle BNB fights in WBNB precisely because a raw payout is a
     *      `.call{value:}` to the winner, and a winner that is a contract with
     *      a reverting `receive()` would have reverted the whole duel — a
     *      griefing vector that settles nothing and kills the fight.
     *
     *      Crediting cannot revert. The winner pulls with `withdraw` in their
     *      own transaction, where a broken `receive()` is their own problem and
     *      nobody else's fight is at risk. This is what lets BNB stay native
     *      all the way through without reopening that hole.
     */
    function _payStake(address asset, address to, uint256 amount) private {
        if (asset == address(0) || amount == 0) return;
        if (asset == address(wbnb)) {
            _creditBnb(to, amount);
            return;
        }
    ...
    /**
     * @notice Recover an ERC-20 sent here by mistake.
     *
     * @dev ⚠ THE OLD SAFETY ARGUMENT NO LONGER HOLDS AND ITS REPLACEMENT IS
     *      LOAD-BEARING. The previous Duel could say "stakes are pulled and
     *      paid out inside one transaction, so this contract holds no player
     *      money between calls and there is nothing here to steal". That is
     *      FALSE here: `bnbCredit` custodies player money indefinitely.
     *
     *      What keeps this safe is narrower and must not be widened: player
     *      money is held as NATIVE BNB, and this function moves ERC-20s only.
     *      There is deliberately still no native rescue, and adding one would
     *      hand the owner every player's fight balance. If a native sweep is
     *      ever wanted for genuinely stray BNB, it must be bounded by
     *      `address(this).balance - totalBnbCredit` and nothing looser.
     *
     *      ⚠ WBNB is not exempted, and does not need to be: the only WBNB this
     *      contract ever holds exists for the few opcodes between the pot-slice
     *      wrap and `fund()`, inside a single transaction. There is no resting
     *      WBNB balance to rescue.
     */
